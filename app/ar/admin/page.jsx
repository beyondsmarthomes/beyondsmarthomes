...
async function markPaid(prevState, formData) {
  'use server'
...
async function cancelOrder(prevState, formData) {
  'use server'
  const id = String(formData.get('id')||'')
              ...
              <tr key={o.id} className="border-t">
                ...
                <td className="p-3 flex gap-2 justify-end">
                  <form action={markPaid}>
                    ...
                    <button className="px-3 py-1 rounded bg-green-600 text-white">تأكيد الدفع</button>
                  </form>
                  <form action={cancelOrder}>
                    <input type="hidden" name="id" defaultValue={o.id} />
                    <button className="px-3 py-1 rounded bg-red-600 text-white">إلغاء</button>
                  </form>
                </td>
              </tr>
            ))}
